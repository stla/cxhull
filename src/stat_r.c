...
void qh_allstatA(qhT* qh) {
  ...
  zdef_(wadd, Wnewvertex, "ave. distance of a new vertex to a facet(!0s)",
        Znewvertex);
  ...
  qh->qhstat.precision =
      qh->qhstat.next; /* call qh_precision for each of these */
  zzdef_(zdoc, Zdoc3, "precision problems (corrected unless 'Q0' or an error)",
         -1);
...
void qh_allstatB(qhT* qh) {
  ...
  zdef_(zinc, Znowsimplicial, "number of simplicial facets that were merged",
        -1);
  ...
  zdef_(wadd, Wangle, "average angle(cosine) of facet normals for all ridges",
        Zangle);
  zdef_(wmax, Wanglemax,
        "  maximum angle(cosine) of facet normals across a ridge", -1);
  zdef_(wmin, Wanglemin,
        "  minimum angle(cosine) of facet normals across a ridge", -1);
...
void qh_allstatD(qhT* qh) {
  ...
  zdef_(zdoc, Zdoc4, "partitioning statistics(see previous for outer planes)",
        -1);
  ...
  zdef_(zinc, Zpartflip,
        "  repartitioned coplanar points for flipped orientation", -1);
}
...
void qh_allstatE2(qhT* qh) {
  ...
  zdef_(zinc, Zhashtests, "average number of tests to match a ridge",
        Zhashlookup);
  zdef_(zinc, Zhashridge, "total lookups of subridges(duplicates and boundary)",
        -1);
...
void qh_allstatH(qhT* qh) {
  ...
  zdef_(zadd, Zmempoints,
        "for input points, outside and coplanar sets, and qhT", -1);
...
void qh_allstatI(qhT* qh) {
  ...
  zdef_(zadd, Ztricoplanartot, "  ave. new facets created(may be deleted)",
        Ztricoplanar);
  zdef_(zmax, Ztricoplanarmax, "  max. new facets created", -1);
  zdef_(zinc, Ztrinull, "null new facets deleted(duplicated vertex)", -1);
  zdef_(zinc, Ztrimirror, "mirrored pairs of new facets deleted(same vertices)",
        -1);
...
void qh_collectstatistics(qhT* qh) {
  ...
  FORALLfacets {
    ...
    if(facet->outsideset)
      zadd_(Zmempoints,
            sizeof(setT) + SETelemsize * qh_setsize(qh, facet->outsideset));
    if(facet->coplanarset)
      zadd_(Zmempoints,
            sizeof(setT) + SETelemsize * qh_setsize(qh, facet->coplanarset));
...
boolT qh_newstats(qhT* qh, int idx, int* nextindex) {
  ...
  if(qh->qhstat.type[qh->qhstat.id[idx]] == zdoc)
    ...
    start = idx;
  for(i = start;
      i < qh->qhstat.next && qh->qhstat.type[qh->qhstat.id[i]] != zdoc; i++) {
    if(!qh_nostatistic(qh, qh->qhstat.id[i]) &&
       !qh->qhstat.printed[qh->qhstat.id[i]])
      isnew = True;
  }
  *nextindex = i;
  return isnew;
...
boolT qh_nostatistic(qhT* qh, int i) {
  if((qh->qhstat.type[i] > ZTYPEreal &&
      qh->qhstat.stats[i].r ==
          qh->qhstat.init[(unsigned char)(qh->qhstat.type[i])].r) ||
     (qh->qhstat.type[i] < ZTYPEreal &&
      qh->qhstat.stats[i].i ==
          qh->qhstat.init[(unsigned char)(qh->qhstat.type[i])].i))
    return True;
  return False;
} /* nostatistic */
...
#define NULLfield "       "
  ...
  if(qh->qhstat.count[id] != -1 &&
    ...
    qh_fprintf(qh, fp, 9363, "%7.2g",
               qh->qhstat.stats[id].r /
