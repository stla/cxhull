...
below %2.2g of the nearest %sfacet.\n",
               qh_setsize(qh, facets), maxoutside,
               (qh->ONLYgood ? "good " : ""));
  FOREACHfacet_i_(qh, facets) { /* for each point with facet assignment */
    ...
    numpart++;
    bestfacet = qh_findbesthorizon(qh, !qh_IScheckmax, point, facet, qh_NOupper,
                                   &dist, &numpart);
    ...
    if(dist > maxoutside) {
      if(qh->ONLYgood && !bestfacet->good &&
        ...
        waserror = True;
        qh_fprintf(qh, qh->ferr, 6109,
                   "qhull precision error: point p%d is outside facet f%d, "
                   "distance= %6.8g maxoutside= %6.8g\n",
...
options 'Qci Tv' to verify all points.\n",
  ...
  if(maxdist > qh->outside_err) {
    ...
    qh_errexit2(qh, qh_ERRprec, errfacet1, errfacet2);
  /* else if waserror, the error was logged to qh.ferr but does not effect the
   * output */
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  Returns
    Level 1 log of the duplicate ridge with the minimum distance between
  vertices Throws error if the merge will increase the maximum facet width by
  qh_WIDEduplicate (100x)
...
void qh_check_dupridge(qhT* qh,
  ...
  qh_outerinner(qh, NULL, &outerplane,
                &innerplane); /* ratio from qh_printsummary */
  ...
  FOREACHvertex_(
      facet1->vertices) { /* The duplicate ridge is between facet1 and facet2,
                             so either facet can be tested */
  ...
  trace0((qh, qh->ferr, 16,
          "qh_check_dupridge: duplicate ridge between f%d and f%d due to "
          "nearly-coincident vertices (%2.2g), dist %2.2g, reverse dist %2.2g, "
          "ratio %2.2g while processing p%d\n",
          facet1->id, facet2->id, minvertex, dist1, dist2, ratio,
          qh->furthest_id));
  if(ratio > qh_WIDEduplicate) {
    qh_fprintf(
        qh, qh->ferr, 6271,
        "qhull precision error (qh_check_dupridge): wide merge (%.0f times "
        "wider) due to duplicate ridge with nearly coincident points (%2.2g) "
        "between f%d and f%d, merge dist %2.2g, while processing p%d\n- Ignore "
        "error with option 'Q12'\n- To be fixed in a later version of Qhull\n",
    ...
    if(minvertex > qh_WIDEduplicate * prevdist)
      qh_fprintf(qh, qh->ferr, 8146,
                 "- Vertex distance %2.2g is greater than %d times maximum "
                 "distance %2.2g\n  Please report to bradb@shore.net with "
                 "steps to reproduce and all output\n",
    ...
    FORALLvertices {
      FOREACHneighbor_(vertex) {
        ...
        if(-dist > qh->TRACEdist || dist > qh->TRACEdist ||
          ...
          qh_fprintf(qh, qh->ferr, 8093,
                     "qh_check_maxout: p%d(v%d) is %.2g from f%d\n",
                     qh_pointid(qh, vertex->point), vertex->id, dist,
                     neighbor->id);
      }
    }
    if(qh->MERGING) {
  ...
  do {
    ...
    FOREACHfacet_i_(qh, facets) { /* for each point with facet assignment */
      if(facet) {
        ...
        qh_distplane(qh, point, facet, &dist);
        numpart++;
        bestfacet = qh_findbesthorizon(qh, qh_IScheckmax, point, facet,
                                       !qh_NOupper, &dist, &numpart);
        ...
        if(dist > qh->TRACEdist || (bestfacet && bestfacet == qh->tracefacet))
          qh_fprintf(qh, qh->ferr, 8094,
                     "qh_check_maxout: p%d is %.2g above f%d\n",
                     qh_pointid(qh, point), dist,
                     (bestfacet ? bestfacet->id : UINT_MAX));
      }
    }
  } while(maxoutside > 2 * old_maxoutside);
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  qh_check_output(qh)
    ...
    Maybe called after voronoi output.  Will recompute otherwise centrums are
  Voronoi centers instead
*/
...
all %sfacets.  Will make %2.0f distance computations.\n",
    ...
    FORALLfacets {
      ...
      FORALLpoints {
        if(point != qh->GOODpointp)
          qh_check_point(qh, point, facet, &maxoutside, &maxdist, &errfacet1,
                         &errfacet2);
      }
    ...
    if(maxdist > qh->outside_err) {
      ...
      qh_errexit2(qh, qh_ERRprec, errfacet1, errfacet2);
    /* else if errfacet1, the error was logged to qh.ferr but does not effect
     * the output */
...
void qh_checkconvex(qhT* qh, facetT* facetlist, int fault) {
  ...
  FORALLfacet_(facetlist) {
    ...
    if(qh->MERGING &&
       (!qh->ZEROcentrum || !facet->simplicial || facet->tricoplanar))
      allsimplicial = False;
    else {
      allsimplicial = True;
      neighbor_i = 0;
      FOREACHneighbor_(facet) {
        ...
        if(dist > -qh->DISTround) {
          ...
          if(dist > qh->DISTround) {
            ...
            qh_fprintf(qh, qh->ferr, 6115,
                       "qhull precision error: f%d is concave to f%d, since "
                       "p%d(v%d) is %6.4g above\n",
                       facet->id, neighbor->id, qh_pointid(qh, vertex->point),
                       vertex->id, dist);
            errfacet1 = facet;
            errfacet2 = neighbor;
            waserror = True;
            ...
            if(dist > 0) { /* qh_checkzero checks that dist < - qh->DISTround */
              ...
              qh_fprintf(qh, qh->ferr, 6116,
                         "qhull precision error: f%d is clearly not convex to "
                         "f%d, since p%d(v%d) is %6.4g above\n",
                         facet->id, neighbor->id, qh_pointid(qh, vertex->point),
                         vertex->id, dist);
              errfacet1 = facet;
              errfacet2 = neighbor;
    ...
    if(!allsimplicial) {
      if(qh->CENTERtype == qh_AScentrum) {
        ...
        if(!centrum_warning && (!facet->simplicial || facet->tricoplanar)) {
          ...
          qh_fprintf(qh, qh->ferr, 7062,
                     "qhull warning: recomputing centrums for convexity test.  "
                     "This may lead to false, precision errors.\n");
        }
      ...
      FOREACHneighbor_(facet) {
        ...
        if(dist > qh->DISTround) {
          zzinc_(Zconcaveridges);
          qh_precision(qh, "concave ridge");
          qh_fprintf(qh, qh->ferr, 6117,
                     "qhull precision error: f%d is concave to f%d.  Centrum "
                     "of f%d is %6.4g above f%d\n",
          ...
          zzinc_(Zcoplanarridges);
          qh_precision(qh, "coplanar ridge");
          qh_fprintf(qh, qh->ferr, 6118,
                     "qhull precision error: f%d is coplanar or concave to "
                     "f%d.  Centrum of f%d is %6.4g above f%d\n",
                     facet->id, neighbor->id, facet->id, dist, neighbor->id);
          errfacet1 = facet;
          errfacet2 = neighbor;
          waserror = True;
...
void qh_checkfacet(qhT* qh, facetT* facet, boolT newmerge, boolT* waserrorp) {
  ...
  if(!facet->normal) {
    qh_fprintf(qh, qh->ferr, 6120,
               "qhull internal error (qh_checkfacet): facet f%d does not have  "
               "a normal\n",
               facet->id);
    waserror = True;
  }
  ...
  FOREACHvertex_(facet->vertices) {
    ...
    if(vertex->id >= previousid) {
      qh_fprintf(qh, qh->ferr, 6122,
                 "qhull internal error (qh_checkfacet): vertices of f%d are "
                 "not in descending id order at v%d\n",
                 facet->id, vertex->id);
      waserror = True;
      break;
    }
  ...
  if(facet->simplicial) {
    if(numvertices + numneighbors != 2 * qh->hull_dim && !facet->degenerate &&
       !facet->redundant) {
      qh_fprintf(qh, qh->ferr, 6123,
                 "qhull internal error (qh_checkfacet): for simplicial facet "
                 "f%d, #vertices %d + #neighbors %d != 2*qh->hull_dim\n",
    ...
    if(!newmerge &&
      ...
      qh_fprintf(qh, qh->ferr, 6124,
                 "qhull internal error (qh_checkfacet): for facet f%d, "
                 "#vertices %d or #neighbors %d < qh->hull_dim\n",
                 facet->id, numvertices, numneighbors);
      waserror = True;
    }
  ...
  FOREACHneighbor_(facet) {
    if(neighbor == qh_MERGEridge || neighbor == qh_DUPLICATEridge) {
      qh_fprintf(qh, qh->ferr, 6126,
                 "qhull internal error (qh_checkfacet): facet f%d still has a "
                 "MERGE or DUP neighbor\n",
                 facet->id);
      qh_errexit(qh, qh_ERRqhull, facet, NULL);
    }
    neighbor->seen = True;
  ...
  FOREACHneighbor_(facet) {
    if(!qh_setin(neighbor->neighbors, facet)) {
      qh_fprintf(qh, qh->ferr, 6127,
                 "qhull internal error (qh_checkfacet): facet f%d has neighbor "
                 "f%d, but f%d does not have neighbor f%d\n",
                 facet->id, neighbor->id, neighbor->id, facet->id);
      errother = neighbor;
      waserror = True;
    }
    if(!neighbor->seen) {
      qh_fprintf(qh, qh->ferr, 6128,
                 "qhull internal error (qh_checkfacet): facet f%d has a "
                 "duplicate neighbor f%d\n",
  ...
  FOREACHridge_(facet->ridges) {
    ...
    if(numRvertices != qh->hull_dim - 1) {
      qh_fprintf(qh, qh->ferr, 6130,
                 "qhull internal error (qh_checkfacet): ridge between f%d and "
                 "f%d has %d vertices\n",
    ...
    if(!qh_setin(facet->neighbors, neighbor)) {
      qh_fprintf(qh, qh->ferr, 6131,
                 "qhull internal error (qh_checkfacet): for facet f%d, "
                 "neighbor f%d of ridge r%d not in facet\n",
  ...
  if(!facet->simplicial) {
    FOREACHneighbor_(facet) {
      if(!neighbor->seen) {
        qh_fprintf(qh, qh->ferr, 6132,
                   "qhull internal error (qh_checkfacet): facet f%d does not "
                   "have a ridge for neighbor f%d\n",
                   facet->id, neighbor->id);
        errother = neighbor;
        waserror = True;
      }
      intersection =
          qh_vertexintersect_new(qh, facet->vertices, neighbor->vertices);
      ...
      FOREACHridge_(facet->ridges) {
        ...
        FOREACHvertex_(ridge->vertices) {
          if(!vertex->seen) {
            qh_fprintf(qh, qh->ferr, 6133,
                       "qhull internal error (qh_checkfacet): vertex v%d in "
                       "r%d not in f%d intersect f%d\n",
 ...
 not in a ridge.  This is ok under merging.  Last point was p%d\n",
    ...
    FOREACHneighbor_(facet) {
      if(neighbor->simplicial) {
        ...
        if(skipA < 0 || skipB < 0 ||
           !qh_setequal_skip(facet->vertices, skipA, neighbor->vertices,
                             skipB)) {
          qh_fprintf(qh, qh->ferr, 6135,
                     "qhull internal error (qh_checkfacet): facet f%d skip %d "
                     "and neighbor f%d skip %d do not match \n",
      ...
      for(i = ridge_i + 1; i < ridge_n; i++) {
        ...
        if(qh_setequal(ridge->vertices, ridge2->vertices)) {
          qh_fprintf(qh, qh->ferr, 6227,
                     "Qhull internal error (qh_checkfacet): ridges r%d and r%d "
...
void qh_checkpolygon(qhT* qh, facetT* facetlist) {
      ...
      if(!nextseen) {
        if(facet == qh->facet_next)
          ...
          if(!qh->NARROWhull
            ...
            qh_fprintf(qh, qh->ferr, 6137,
                       "qhull internal error (qh_checkpolygon): f%d has "
                       "outside points before qh->facet_next\n",
                       facet->id);
            qh_errexit(qh, qh_ERRqhull, facet, NULL);
          }
        }
  ...
  if(qh->visible_list && !visibleseen && facetlist == qh->facet_list) {
    qh_fprintf(qh, qh->ferr, 6138,
               "qhull internal error (qh_checkpolygon): visible list f%d no "
               "longer on facet list\n",
               qh->visible_list->id);
    qh_printlists(qh);
    ...
    FOREACHvertex_(facet->vertices) {
      ...
      if(!vertex->seen) {
        ...
        if(qh_pointid(qh, vertex->point) == qh_IDunknown) {
          qh_fprintf(qh, qh->ferr, 6139,
                     "qhull internal error (qh_checkpolygon): unknown point %p "
                     "for vertex v%d first_point %p\n",
  ...
  if(facetlist == qh->facet_list) {
    if(numfacets != qh->num_facets - qh->num_visible) {
      qh_fprintf(
          ...
          "%d, cumulative facet count is %d - %d visible facets\n",
    ...
    if(qh->VERTEXneighbors) {
      ...
      FORALLfacet_(facetlist) totvertices += qh_setsize(qh, facet->vertices);
      if(totvneighbors != totvertices) {
        qh_fprintf(qh, qh->ferr, 6141,
    ...
    if(numvertices != qh->num_vertices - qh_setsize(qh, qh->del_vertices)) {
      qh_fprintf(qh, qh->ferr, 6142,
                 "qhull internal error (qh_checkpolygon): actual number of "
                 "vertices is %d, cumulative vertex count is %d\n",
                 numvertices,
    ...
    if(qh->hull_dim == 2 && numvertices != numfacets) {
      qh_fprintf(qh, qh->ferr, 6143,
                 "qhull internal error (qh_checkpolygon): #vertices %d != "
                 "#facets %d\n",
                 numvertices, numfacets);
      waserror = True;
    }
    if(qh->hull_dim == 3 && numvertices + numfacets - numridges / 2 != 2) {
      qh_fprintf(qh, qh->ferr, 7063,
    ...
    if(qh_setsize(qh, vertex->neighbors)) {
      FOREACHneighbor_(vertex) {
        if(!qh_setin(neighbor->vertices, vertex)) {
          qh_fprintf(qh, qh->ferr, 6146,
                     "qhull internal error (qh_checkvertex): neighbor f%d does "
                     "not contain v%d\n",
...
void qh_createsimplex(qhT* qh, setT* vertices) {
  ...
  qh->vertex_list = qh->newvertex_list = qh->vertex_tail =
      qh_newvertex(qh, NULL);
  FOREACHvertex_i_(qh, vertices) {
    newfacet = qh_newfacet(qh);
    newfacet->vertices =
        qh_setnew_delnthsorted(qh, vertices, vertex_n, vertex_i, 0);
...
setT* qh_facet3vertex(qhT* qh, facetT* facet) {
  ...
  if(facet->simplicial) {
    ...
    if(!ridge || cntprojected != cntvertices) {
      qh_fprintf(qh, qh->ferr, 6148,
                 "qhull internal error (qh_facet3vertex): ridges for facet %d "
                 "don't match up.  got at least %d\n",
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  notes:
    ...
    different tricoplanar facet. See <a
  href="../html/qh-code.htm#findfacet">locate a facet with
  qh_findbestfacet()</a>
...
facetT* qh_findbestfacet(qhT* qh,
  ...
  bestfacet =
      qh_findbest(qh, point, qh->facet_list, bestoutside, !qh_ISnewfacets,
  ...
  if(*bestdist < -qh->DISTround) {
    ...
    if((isoutside && *isoutside && bestoutside) ||
      ...
      bestfacet = qh_findbest(qh, point, bestfacet, bestoutside, False,
                              bestoutside, bestdist, isoutside, &totpart);
      totpart += numpart;
    }
  }
  trace3((qh, qh->ferr, 3014,
          "qh_findbestfacet: f%d dist %2.2g isoutside %d totpart %d\n",
          bestfacet->id, *bestdist, (isoutside ? *isoutside : UINT_MAX),
          totpart));
  return bestfacet;
} /* findbestfacet */
...
facetT* qh_findbestlower(qhT* qh,
  ...
  if(!bestfacet) {
    ...
    trace3((qh, qh->ferr, 3025,
            "qh_findbestlower: all neighbors of facet %d are flipped or upper "
            "Delaunay.  Search all facets\n",
            upperfacet->id));
    /* rarely called */
    bestfacet = qh_findfacet_all(qh, point, &bestdist, &isoutside, numpart);
  }
  ...
  if(qh->GOODvertex < 0 || (qh->GOODvertex > 0 && qh->MERGING)) {
    FORALLfacet_(facetlist) {
      if(facet->good && ((qh->GOODvertex > 0) ^
        ...
        if(!--numgood) {
          if(qh->ONLYgood) {
            qh_fprintf(qh, qh->ferr, 7064,
                       "qhull warning: good vertex p%d does not match last "
                       "good facet f%d.  Ignored.\n",
            ...
            qh_fprintf(qh, qh->ferr, 7066,
                       "qhull warning: point p%d is a vertex for every "
                       "facet('QV-%d').\n",
                       -qh->GOODvertex - 1, -qh->GOODvertex - 1);
        }
        facet->good = False;
...
void qh_initbuild(qhT* qh) {
  ...
  if(qh->DELAUNAY && qh->upper_threshold[qh->hull_dim - 1] > REALmax / 2 &&
    ...
    if(i < 0) {
      if(qh->UPPERdelaunay) { /* matches qh.upperdelaunay in qh_setfacetplane */
        qh->lower_threshold[qh->hull_dim - 1] =
            qh->ANGLEround * qh_ZEROdelaunay;
        qh->GOODthreshold = True;
      } else {
        qh->upper_threshold[qh->hull_dim - 1] =
            -qh->ANGLEround * qh_ZEROdelaunay;
        if(!qh->GOODthreshold)
  ...
  vertices = qh_initialvertices(qh, qh->hull_dim, maxpoints, qh->first_point,
                                qh->num_points);
  ...
  if(qh->PRINToptions1st || qh->TRACElevel || qh->IStracing) {
    if(qh->TRACElevel || qh->IStracing)
      qh_fprintf(qh, qh->ferr, 8103, "\nTrace level %d for %s | %s\n",
                 qh->IStracing ? qh->IStracing : qh->TRACElevel,
  ...
  qh_resetlists(
      qh, False,
      qh_RESETvisible /*qh.visible_list newvertex_list newfacet_list */);
  ...
  if(qh->ONLYgood) {
    if(qh->GOODvertex > 0 && qh->MERGING) {
      qh_fprintf(qh, qh->ferr, 6151,
                 "qhull input error: 'Qg QVn' (only good vertex) does not work "
                 "with merging.\nUse 'QJ' to joggle the input or 'Q0' to turn "
                 "off merging.\n");
      qh_errexit(qh, qh_ERRinput, NULL, NULL);
    }
...
good point(QGn or QG-n), or a good vertex with 'QJ' or 'Q0' (QVn).\n");
    ...
    if(qh->GOODvertex > 0 && !qh->MERGING /* matches qh_partitionall */
      ...
      facet = qh_findbestnew(qh, qh->GOODvertexp, qh->facet_list, &dist,
                             !qh_ALL, &isoutside, &numpart);
      zadd_(Zdistgood, numpart);
      if(!isoutside) {
        qh_fprintf(qh, qh->ferr, 6153,
                   "qhull input error: point for QV%d is inside initial "
                   "simplex.  It can not be made a vertex.\n",
...
void qh_initialhull(qhT* qh, setT* vertices) {
  ...
  FORALLfacets qh_setfacetplane(qh, facet);
  FORALLfacets {
    if(!qh_checkflipped(qh, facet, NULL,
                        qh_ALL)) { /* due to axis-parallel facet */
      trace1((qh, qh->ferr, 1031,
  ...
  FORALLfacets {
    if(!qh_checkflipped(qh, facet, NULL,
                        !qh_ALL)) { /* can happen with 'R0.1' */
      if(qh->DELAUNAY && !qh->ATinfinity) {
        if(qh->UPPERdelaunay)
          qh_fprintf(qh, qh->ferr, 6240,
                     "Qhull precision error: Initial simplex is cocircular or "
                     "cospherical.  Option 'Qs' searches all points.  Can not "
                     "compute the upper Delaunay triangulation or upper "
                     "Voronoi diagram of cocircular/cospherical points.\n");
        else
          qh_fprintf(
              qh, qh->ferr, 6239,
              "Qhull precision error: Initial simplex is cocircular or "
              "cospherical.  Use option 'Qz' for the Delaunay triangulation or "
              "Voronoi diagram of cocircular/cospherical points.  Option 'Qz' "
              "adds a point \"at infinity\".  Use option 'Qs' to search all "
              "points for the initial simplex.\n");
        qh_errexit(qh, qh_ERRinput, NULL, NULL);
      }
      qh_precision(qh, "initial simplex is flat");
      qh_fprintf(qh, qh->ferr, 6154,
                 "Qhull precision error: Initial simplex is flat (facet %d is "
                 "coplanar with the interior point)\n",
                 facet->id);
      qh_errexit(qh, qh_ERRsingular, NULL,
                 NULL); /* calls qh_printhelp_singular */
    }
    FOREACHneighbor_(facet) {
...
setT* qh_initialvertices(qhT* qh,
  ...
  if(qh->ALLpoints)
    ...
    qh_maxsimplex(qh, fmin_(qh_INITIALsearch, dim), maxpoints, points,
                  numpoints, &simplex);
    ...
    qh_maxsimplex(qh, dim, maxpoints, points, numpoints, &simplex);
  FOREACHpoint_(simplex) qh_setaddnth(
      qh, &vertices, 0, qh_newvertex(qh, point)); /* descending order */
...
void qh_matchduplicates(qhT* qh,
  ...
  for(makematch = 0; makematch < 2; makematch++) {
    ...
    for(newfacet = atfacet, newskip = atskip; newfacet;
      ...
      for(scan = hash; (facet = SETelemt_(qh->hash_table, scan, facetT));
        ...
        if(!facet->dupridge || facet->visitid == qh->visit_id)
          continue;
        zinc_(Zhashtests);
        if(qh_matchvertices(qh, 1, newfacet->vertices, newskip, facet->vertices,
                            &skip, &same)) {
        ...
        if(makematch && !facet &&
          ...
          qh_fprintf(qh, qh->ferr, 6156,
                     "qhull internal error (qh_matchduplicates): no MERGEridge "
                     "match for duplicate f%d skip %d at hash %d\n",
    ...
    if(!makematch) {
      if(!maxmatch) {
        qh_fprintf(qh, qh->ferr, 6157,
                   "qhull internal error (qh_matchduplicates): no maximum "
                   "match at duplicate f%d skip %d at hash %d\n",
                   atfacet->id, atskip, hash);
        qh_errexit(qh, qh_ERRqhull, atfacet, NULL);
      }
      SETelem_(maxmatch->neighbors, maxskip) =
          maxmatch2; /* maxmatch!=0 by QH6157 */
...
vertexT* qh_nearvertex(qhT* qh,
  ...
  if(facet->tricoplanar) {
    if(!qh->VERTEXneighbors || !facet->center) {
      qh_fprintf(qh, qh->ferr, 6158,
                 "qhull internal error (qh_nearvertex): qh.VERTEXneighbors and "
                 "facet->center required for tricoplanar facets\n");
      qh_errexit(qh, qh_ERRqhull, facet, NULL);
    }
  ...
  *bestdistp = sqrt(bestdist);
  if(!bestvertex) {
    qh_fprintf(qh, qh->ferr, 6261,
               "qhull internal error (qh_nearvertex): did not find bestvertex "
               "for f%d p%d\n",
               facet->id, qh_pointid(qh, point));
    qh_errexit(qh, qh_ERRqhull, facet, NULL);
  }
  trace3((qh, qh->ferr, 3019, "qh_nearvertex: v%d dist %2.2g for f%d p%d\n",
          bestvertex->id, *bestdistp, facet->id,
          qh_pointid(qh, point))); /* bestvertex!=0 by QH2161 */
  return bestvertex;
} /* nearvertex */
...
int qh_newhashtable(qhT* qh, int newsize) {
  ...
  while(True) {
    if(newsize < 0 || size < 0) {
      qh_fprintf(qh, qh->qhmem.ferr, 6236,
                 "qhull error (qh_newhashtable): negative request (%d) or size "
                 "(%d).  Did int overflow due to high-D?\n",
                 newsize, size); /* WARN64 */
...
vertexT* qh_newvertex(qhT* qh, pointT* point) {
  ...
  if(qh->vertex_id == UINT_MAX) {
    qh_memfree(qh, vertex, (int)sizeof(vertexT));
    qh_fprintf(qh, qh->ferr, 6159,
               "qhull error: more than 2^32 vertices.  vertexT.id field "
               "overflows.  Vertices would not be sorted correctly.\n");
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  }
...
void qh_printlists(qhT* qh) {
  ...
  qh_fprintf(qh, qh->ferr, 8111,
             "\n  new facets %d visible facets %d next facet for qh_addpoint "
             "%d\n  vertices(new %d):",
             getid_(qh->newfacet_list), getid_(qh->visible_list),
             getid_(qh->facet_next), getid_(qh->newvertex_list));
  count = 0;
  FORALLvertices {
    if(++count % 100 == 0)
...
void qh_resetlists(
  ...
  qh->visible_list =
      NULL; /* may still have visible facets via qh_triangulate */
  qh->NEWfacets = False;
} /* resetlists */
...
void qh_triangulate(qhT* qh /*qh.facet_list*/) {
  ...
  if(qh->hasTriangulation)
    return;
  trace1((qh, qh->ferr, 1034,
          "qh_triangulate: triangulate non-simplicial facets\n"));
  if(qh->hull_dim == 2)
    return;
  if(qh->VORONOI) { /* otherwise lose Voronoi centers [could rebuild vertex set
                       from tricoplanar] */
    qh_clearcenters(qh, qh_ASvoronoi);
    qh_vertexneighbors(qh);
  }
  ...
  trace2((qh, qh->ferr, 2047,
          "qh_triangulate: delete null facets from f%d -- apex same as second "
          "vertex\n",
          getid_(new_facet_list)));
  for(facet = new_facet_list; facet && facet->next;
      facet = nextfacet) { /* null facets moved to end */
    nextfacet = facet->next;
    if(facet->visible)
      continue;
    if(facet->ridges) {
  ...
  trace2((qh, qh->ferr, 2048,
          "qh_triangulate: delete %d or more mirror facets -- same vertices "
          "and neighbors\n",
  ...
  qh_updatevertices(
      qh /*qh.newvertex_list, empty newfacet_list and visible_list*/);
  qh_resetlists(qh, False, !qh_RESETvisible /*qh.newvertex_list, empty newfacet_list and visible_list*/);

  trace2((qh, qh->ferr, 2050,
          "qh_triangulate: identify degenerate tricoplanar facets from f%d\n",
          getid_(new_facet_list)));
  trace2((qh, qh->ferr, 2051,
          "qh_triangulate: and replace facet->f.triowner with tricoplanar "
          "facets that own center, normal, etc.\n"));
  ...
  trace2((qh, qh->ferr, 2052,
          "qh_triangulate: delete visible facets -- non-simplicial, null, and "
          "mirrored facets\n"));
  owner = NULL;
  visible = NULL;
  for(facet = new_facet_list; facet && facet->next;
      facet = nextfacet) { /* may delete facet */
    nextfacet = facet->next;
    if(facet->visible) {
      if(facet->tricoplanar) { /* a null or mirrored facet */
        ...
        if(visible && !owner) {
          ...
          trace2((qh, qh->ferr, 2053,
                  "qh_triangulate: all tricoplanar facets degenerate for "
                  "non-simplicial facet f%d\n",
                  visible->id));
      ...
      if(facet->f.triowner != visible || visible == NULL) {
        qh_fprintf(qh, qh->ferr, 6162,
                   "qhull error (qh_triangulate): tricoplanar facet f%d not "
                   "owned by its visible, non-simplicial facet f%d\n",
  ...
  if(visible && !owner) {
    trace2((qh, qh->ferr, 2054,
            "qh_triangulate: all tricoplanar facets degenerate for last "
            "non-simplicial facet f%d\n",
            visible->id));
    qh_delfacet(qh, visible);
    qh->num_visible--;
...
void qh_triangulate_facet(qhT* qh, facetT* facetA, vertexT** first_vertex) {
  ...
  if(qh->CENTERtype == qh_ASvoronoi &&
     ...
     && fabs_(facetA->normal[qh->hull_dim - 1]) >=
            qh->ANGLEround * qh_ZEROdelaunay) {
  ...
  qh->newvertex_list = NULL;
  qh_updatevertices(
      qh /*qh.newfacet_list, qh.empty visible_list and qh.newvertex_list*/);
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  design:
    if they are already neighbors, the opposing neighbors become MRGmirror
  facets
*/
void qh_triangulate_link(qhT* qh,
  ...
  trace3((qh, qh->ferr, 3021,
          "qh_triangulate_link: relink old facets f%d and f%d between "
          "neighbors f%d and f%d\n",
  ...
  if(qh_setin(facetA->neighbors, facetB)) {
    ...
    errmirror = True;
  if(errmirror) {
    qh_fprintf(qh, qh->ferr, 6163,
               "qhull error (qh_triangulate_link): mirror facets f%d and f%d "
               "do not match for old facets f%d and f%d\n",
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  design:
    since a null facet duplicates the first two vertices, the opposing neighbors
  absorb the null facet if they are already neighbors, the opposing neighbors
  become MRGmirror facets
*/
