/*<html><pre>  -<a                             href="qh-merge_r.htm#TOC"
  >-------------------------------</a><a name="TOP">-</a>
   ...
   To remove deleted facets and vertices (qhull() in libqhull_r.c):
     qh_partitionvisible(qh, !qh_ALL, &numoutside);  // visible_list,
  newfacet_list qh_deletevisible();         // qh.visible_list qh_resetlists(qh,
  False, qh_RESETvisible);       // qh.visible_list newvertex_list newfacet_list
...
void qh_appendmergeset(qhT* qh,
  ...
  if(mergetype < MRGdegen)
    ...
    if(facet->redundant || neighbor->redundant) {
      qh_fprintf(qh, qh->ferr, 6092,
                 "qhull error (qh_appendmergeset): facet f%d or f%d is already "
                 "a mirrored facet\n",
                 facet->id, neighbor->id);
      qh_errexit2(qh, qh_ERRqhull, facet, neighbor);
    }
    if(!qh_setequal(facet->vertices, neighbor->vertices)) {
      qh_fprintf(qh, qh->ferr, 6093,
                 "qhull error (qh_appendmergeset): mirrored facets f%d and f%d "
                 "do not have the same vertices\n",
                 facet->id, neighbor->id);
      qh_errexit2(qh, qh_ERRqhull, facet, neighbor);
    }
    facet->redundant = True;
...
boolT qh_checkzero(qhT* qh, boolT testall) {
  ...
  trace2((
      qh, qh->ferr, 2012, "qh_checkzero: testall %d, facets are %s\n", testall,
      (qh->MERGEexact && !testall) ? "not concave, flipped, or duplicate ridged"
                                   : "clearly convex"));
  return True;
...
vertexT* qh_find_newvertex(qhT* qh,
...
#ifndef qh_NOtrace
  if(qh->IStracing >= 4) {
    qh_fprintf(qh, qh->ferr, 8063,
               "qh_find_newvertex: find new vertex for v%d from ",
               oldvertex->id);
    FOREACHvertex_(vertices) qh_fprintf(qh, qh->ferr, 8064, "v%d ", vertex->id);
    FOREACHridge_(ridges) qh_fprintf(qh, qh->ferr, 8065, "r%d ", ridge->id);
  ...
  FOREACHvertex_(vertices) {
    ...
    FOREACHridge_(newridges) {
      if(qh_hashridge_find(qh, qh->hash_table, hashsize, ridge, vertex,
                           oldvertex, &hash)) {
        zinc_(Zdupridge);
        break;
      }
  ...
  if(vertex) {
    ...
    trace2((qh, qh->ferr, 2020,
            "qh_find_newvertex: found v%d for old v%d from %d vertices and %d "
            "ridges.\n",
            vertex->id, oldvertex->id, qh_setsize(qh, vertices),
            qh_setsize(qh, ridges)));
  } else {
    zinc_(Zfindfail);
    trace0((qh, qh->ferr, 14,
...
/*-<a                             href="qh-merge_r.htm#TOC"
  ...
  qh_findbest_test(qh, testcentrum, facet, neighbor, bestfacet, dist, mindist,
...
facetT* qh_findbestneighbor(qhT* qh,
  ...
  if(qh->CENTERtype == qh_ASvoronoi) {
    qh_fprintf(qh, qh->ferr, 6272,
               "qhull error: cannot call qh_findbestneighor for f%d while "
               "qh.CENTERtype is qh_ASvoronoi\n",
               facet->id);
  ...
  if(size > qh->hull_dim + qh_BESTnonconvex) {
    FOREACHridge_(facet->ridges) {
      if(ridge->nonconvex) {
        ...
        qh_findbest_test(qh, testcentrum, facet, neighbor, &bestfacet, distp,
                         mindistp, maxdistp);
      }
    }
  }
...
void qh_getmergeset(qhT* qh, facetT* facetlist) {
  ...
  nummerges = qh_setsize(qh, qh->facet_mergeset);
  if(qh->ANGLEmerge)
    qsort(SETaddr_(qh->facet_mergeset, mergeT), (size_t)nummerges,
          sizeof(mergeT*), qh_compareangle);
  else
    qsort(SETaddr_(qh->facet_mergeset, mergeT), (size_t)nummerges,
          sizeof(mergeT*), qh_comparemerge);
  if(qh->POSTmerging) {
...
void qh_hashridge(qhT* qh,
  ...
  hash =
      qh_gethash(qh, hashsize, ridge->vertices, qh->hull_dim - 1, 0, oldvertex);
  while(True) {
...
/*-<a                             href="qh-merge_r.htm#TOC"
  ...
  ) returns matching ridge without oldvertex in hashtable for ridge without
  vertex if oldvertex is NULL matches with any one skip
...
void qh_makeridges(qhT* qh, facetT* facet) {
  ...
  if(!facet->simplicial)
    return;
  trace4(
      (qh, qh->ferr, 4027, "qh_makeridges: make ridges for f%d\n", facet->id));
  ...
  FOREACHneighbor_i_(qh, facet) {
    if(neighbor == qh_MERGEridge)
      ...
      ridge->vertices = qh_setnew_delnthsorted(qh, facet->vertices,
                                               qh->hull_dim, neighbor_i, 0);
...
void qh_mark_dupridges(qhT* qh, facetT* facetlist) {
  ...
  trace4(
      (qh, qh->ferr, 4028, "qh_mark_dupridges: identify duplicate ridges\n"));
  FORALLfacet_(facetlist) {
    if(facet->dupridge) {
      FOREACHneighbor_(facet) {
        if(neighbor == qh_MERGEridge) {
...
int qh_merge_degenredundant(qhT* qh) {
  ...
  while((merge = (mergeT*)qh_setdellast(qh->degen_mergeset))) {
    ...
    if(mergetype == MRGredundant) {
      ...
      while(facet2->visible) {
        if(!facet2->f.replace) {
          qh_fprintf(qh, qh->ferr, 6097,
                     "qhull internal error (qh_merge_degenredunant): f%d "
                     "redundant but f%d has no replacement\n",
                     facet1->id, facet2->id);
          qh_errexit2(qh, qh_ERRqhull, facet1, facet2);
        }
      ...
      if(!(size = qh_setsize(qh, facet1->neighbors))) {
        ...
        bestneighbor =
            qh_findbestneighbor(qh, facet1, &dist, &mindist, &maxdist);
        trace2((qh, qh->ferr, 2028,
                "qh_merge_degenredundant: facet f%d has %d neighbors, merge "
                "into f%d dist %2.2g\n",
                facet1->id, size, bestneighbor->id, dist));
        qh_mergefacet(qh, facet1, bestneighbor, &mindist, &maxdist,
                      !qh_MERGEapex);
        nummerges++;
        if(qh->PRINTstatistics) {
          zinc_(Zdegen);
...
void qh_merge_nonconvex(qhT* qh,
  ...
  if(dist < dist2) {
    qh_mergefacet(qh, bestfacet, bestneighbor, &mindist, &maxdist,
                  !qh_MERGEapex);
...
void qh_mergecycle(qhT* qh, facetT* samecycle, facetT* newfacet) {
  ...
  if(newfacet->tricoplanar) {
    if(!qh->TRInormals) {
      qh_fprintf(qh, qh->ferr, 6224,
                 "Qhull internal error (qh_mergecycle): does not work for "
                 "tricoplanar facets.  Use option 'Q11'\n");
  ...
  if(newfacet == qh->tracefacet) {
    ...
    qh->IStracing = 4;
    qh_fprintf(qh, qh->ferr, 8068,
               "qh_mergecycle: ========= trace merge %d of samecycle %d into "
               "trace f%d, furthest is p%d\n",
...
void qh_mergecycle_facets(qhT* qh, facetT* samecycle, facetT* newfacet) {
  ...
  trace4((qh, qh->ferr, 4030,
          "qh_mergecycle_facets: make newfacet new and samecycle deleted\n"));
  qh_removefacet(qh,
                 newfacet); /* append as a newfacet to end of qh->facet_list */
...
void qh_mergecycle_ridges(qhT* qh, facetT* samecycle, facetT* newfacet) {
  ...
  trace4(
      (qh, qh->ferr, 4034, "qh_mergecycle_ridges: add ridges to newfacet\n"));
  FORALLsame_cycle_(samecycle) {
    ...
    FOREACHneighbor_i_(qh, same) { /* note: !newfact->simplicial */
      if(neighbor->visitid != samevisitid && neighbor->simplicial) {
        ridge = qh_newridge(qh);
        ridge->vertices = qh_setnew_delnthsorted(qh, same->vertices,
                                                 qh->hull_dim, neighbor_i, 0);
...
void qh_mergefacet(qhT* qh,
  ...
  if(facet1->tricoplanar || facet2->tricoplanar) {
    if(!qh->TRInormals) {
      qh_fprintf(qh, qh->ferr, 6226,
                 "Qhull internal error (qh_mergefacet): does not work for "
                 "tricoplanar facets.  Use option 'Q11'\n");
      qh_errexit2(qh, qh_ERRqhull, facet1, facet2);
    }
  ...
  if(qh->build_cnt >= qh->RERUN) {
    if(mindist && (-*mindist > qh->TRACEdist || *maxdist > qh->TRACEdist)) {
      ...
      traceonce = True;
      qh_fprintf(qh, qh->ferr, 8075,
                 "qh_mergefacet: ========= trace wide merge #%d(%2.2g) for f%d "
                 "into f%d, last point was p%d\n",
                 zzval_(Ztotmerge), fmax_(-*mindist, *maxdist), facet1->id,
                 facet2->id, qh->furthest_id);
    } else if(facet1 == qh->tracefacet || facet2 == qh->tracefacet) {
      tracerestore = qh->IStracing;
      qh->IStracing = 4;
      traceonce = True;
      qh_fprintf(qh, qh->ferr, 8076,
                 "qh_mergefacet: ========= trace merge #%d involving f%d, "
                 "furthest is p%d\n",
  ...
  if(qh->IStracing >= 2) {
    ...
    qh_fprintf(qh, qh->ferr, 8077,
               "qh_mergefacet: #%d merge f%d into f%d, mindist= %2.2g, "
               "maxdist= %2.2g\n",
               zzval_(Ztotmerge), facet1->id, facet2->id, mergemin, mergemax);
  }
#endif /* !qh_NOtrace */
  if(facet1 == facet2 || facet1->visible || facet2->visible) {
    qh_fprintf(qh, qh->ferr, 6099,
               "qhull internal error (qh_mergefacet): either f%d and f%d are "
               "the same or one is a visible facet\n",
               facet1->id, facet2->id);
    qh_errexit2(qh, qh_ERRqhull, facet1, facet2);
  }
...
too strong.\n",
  ...
  qh_removefacet(qh,
                 facet2); /* append as a newfacet to end of qh->facet_list */
...
void qh_mergevertices(qhT* qh, setT* vertices1, setT** vertices2) {
  int newsize =
      qh_setsize(qh, vertices1) + qh_setsize(qh, *vertices2) - qh->hull_dim + 1;
  setT* mergedvertices;
  ...
  if(newsize < qh_setsize(qh, mergedvertices)) {
    qh_fprintf(qh, qh->ferr, 6100,
               "qhull internal error (qh_mergevertices): facets did not share "
               "a ridge\n");
...
setT* qh_neighbor_intersections(qhT* qh, vertexT* vertex) {
  ...
  if(!neighborB)
    ...
    intersect =
        qh_vertexintersect_new(qh, neighborA->vertices, neighborB->vertices);
...
vertexT* qh_rename_sharedvertex(qhT* qh, vertexT* vertex, facetT* facet) {
  ...
  if(qh_setsize(qh, vertex->neighbors) == 2) {
    ...
    if(!neighborA) {
      qh_fprintf(qh, qh->ferr, 6101,
                 "qhull internal error (qh_rename_sharedvertex): v%d's "
                 "neighbors not in f%d\n",
                 vertex->id, facet->id);
  ...
  trace2(
      (qh, qh->ferr, 2037,
       ...
       qh_pointid(qh, vertex->point), vertex->id, facet->id,
       qh_setsize(qh, ridges), neighborA->id));
  zinc_(Zintersectnum);
...
void qh_renamevertex(qhT* qh,
  ...
  if(qh->IStracing >= 2 || oldvertex->id == qh->tracevertex_id ||
    ...
    istrace = True;
  FOREACHridge_(ridges) qh_renameridgevertex(qh, ridge, oldvertex, newvertex);
  if(!oldfacet) {
    zinc_(Zrenameall);
    if(istrace)
      qh_fprintf(qh, qh->ferr, 8082,
...
void qh_tracemerge(qhT* qh, facetT* facet1, facetT* facet2) {
  ...
  if(facet2 == qh->tracefacet ||
     (qh->tracevertex && qh->tracevertex->newlist)) {
    qh_fprintf(qh, qh->ferr, 8085,
               "qh_tracemerge: trace facet and vertex after merge of f%d and "
               "f%d, furthest p%d\n",
               facet1->id, facet2->id, qh->furthest_id);
    if(facet2 != qh->tracefacet)
...
At %d:%d:%d & %2.5g CPU secs, qhull has merged %d facets.  The hull\n\
  contains %d facets and %d vertices.\n",
             tp->tm_hour, tp->tm_min, tp->tm_sec, cpu, total,
