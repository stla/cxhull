...
/*-<a                             href="qh-user_r.htm#TOC"
  ...
  ) Run qhull and return results in qh. Returns exitcode (0 if no errors).
...
int qh_new_qhull(qhT* qh,
                 ...
                 FILE* outfile,
                 FILE* errfile) {
  /* gcc may issue a "might be clobbered" warning for dim, points, and ismalloc
     [-Wclobbered]. These parameters are not referenced after a longjmp() and
     hence not clobbered. See
  ...
  if(strncmp(qhull_cmd, "qhull ", (size_t)6)) {
    qh_fprintf(qh, errfile, 6186,
               "qhull error (qh_new_qhull): start qhull_cmd argument with "
               "\"qhull \"\n");
    return qh_ERRinput;
  }
  qh_initqhull_start(qh, NULL, outfile, errfile);
  trace1((qh, qh->ferr, 1044,
  ...
  if(!exitcode) {
    ...
    if(qh->HALFspace) {
      ...
      new_points =
          qh_sethalfspace_all(qh, dim, numpoints, points, qh->feasible_point);
      new_ismalloc = True;
      if(ismalloc)
        qh_free(points);
    } else {
...
void qh_errexit(qhT* qh, int exitcode, facetT* facet, ridgeT* ridge) {
  ...
  if(qh->furthest_id >= 0) {
    qh_fprintf(qh, qh->ferr, 8129, "Last point added to hull was p%d.",
               qh->furthest_id);
    if(zzval_(Ztotmerge))
      qh_fprintf(qh, qh->ferr, 8130, "  Last merge was #%d.",
                 zzval_(Ztotmerge));
    if(qh->QHULLfinished)
  ...
  if(qh->NOerrexit) {
    qh_fprintf(qh, qh->ferr, 6187,
               "qhull error while ending program, or qh->NOerrexit not cleared "
               "after setjmp(). Exit program with error.\n");
    qh_exit(qh_ERRqhull);
  }
...
to the center point are as follows:\n\n",
               qh->DISTround);
  qh_printpointid(qh, fp, "center point", qh->hull_dim, qh->interior_point,
                  qh_IDunknown);
