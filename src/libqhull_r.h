/*<html><pre>  -<a                             href="qh-qhull_r.htm"
  >-------------------------------</a><a name="TOP">-</a>
   ...
   includes function prototypes for libqhull_r.c, geom_r.c, global_r.c, io_r.c,
  user.c
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  notes:
    qh.STOPcone and qh.STOPpoint assume that qh_IDunknown==-1 (other negative
  numbers indicate points) qh_IDunknown is also returned by getid_() for unknown
  facet, ridge, or vertex
*/
...
typedef enum {
  ...
  qh_ASvoronoi,  /* Set by qh_clearcenters on qh_prepare_output, or if not
                    MERGING and VORONOI */
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  geometric information:
    ...
    f.center            centrum for testing convexity or Voronoi center for
  output f.simplicial        True if facet is simplicial f.flipped True if facet
  does not include qh.interior_point
...
#define qh_MAXnummerge \
  511                    /*     2^9-1, 32 flags total, see "flags:" in io_r.c */
  flagT tricoplanar : 1; /* True if TRIangulate and simplicial and coplanar with
                            a neighbor */
  ...
  flagT mergeridge2 : 1;  /* True if neighbor contains a qh_MERGEridge (qhT *qh,
                             mark_dupridges */
...
struct vertexT {
  ...
  unsigned
      id; /* unique identifier.  Same size as qh.vertex_id and qh.ridge_id */
...
/*-<a                             href="qh-globa_r.htm#TOC"
  ...
  qhT
   All global variables for qhull are in qhT.  It includes qhmemT, qhstatT, and
  rbox globals
...
#define QHULL_LIB_CHECK                                                      \
  qh_lib_check(QHULL_LIB_TYPE, sizeof(qhT), sizeof(vertexT), sizeof(ridgeT), \
               sizeof(facetT), sizeof(setT), sizeof(qhmemT));
...
struct qhT {
  /*-<a                             href="qh-globa_r.htm#TOC"
    ...
    notes:
      ...
      copied into qh by qh_setflags().  qh-quick_r.htm#options defines the
    flags.
  */
  ...
  boolT
      NOwide; /* true 'Q12' if no error on wide merge due to duplicate ridge */
  ...
  int STOPpoint;  /* 'TVn' 'TV-n' 1+n for stopping after/before(-)
                                adding point n */
  int TESTpoints; /* 'QTn' num of test points after qh.num_points.  Test points
                     always coplanar. */
  ...
  boolT TRInormals;     /* true 'Q11' if triangulate duplicates ->normal and
                           ->center (sets Qt) */
  ...
  boolT DOcheckmax; /* true if calling qh_check_maxout (qhT *qh,
                       qh_initqhull_globals) */
  char*
      feasible_string; /* feasible point 'Hn,n,n' for halfspace intersection */
  coordT* feasible_point; /*    as coordinates, both malloc'd */
  boolT GETarea;        /* true 'Fa', 'FA', 'FS', 'PAn', 'PFn' if compute facet
                           area/Voronoi volume in io_r.c */
  ...
  */
  char qhull[sizeof(
      "qhull")];    /* "qhull" for checking ownership while debugging */
  jmp_buf errexit;  /* exit label for qh_errexit, defined by setjmp() and
                       NOerrexit */
  char jmpXtra[40]; /* extra bytes in case jmp_buf is defined wrong by compiler
                     */
  jmp_buf restartexit; /* restart label for qh_errexit, defined by setjmp() and
                          ALLOWrestart */
  ...
  */
  ...
  int num_outside;         /* number of points in outsidesets (for tracing and
                              RANDOMoutside)         includes coplanar outsideset points for
                              NARROWhull/qh_outcoplanar() */
  ...
  */
  ...
  boolT
      NOerrexit; /* true if qh.errexit is not available, cleared after setjmp */
  ...
  unsigned int vertex_visit; /* unique ID for searching vertices, reset with
                                qh_buildtracing */
  boolT ZEROall_ok;          /* True if qh_checkzero always succeeds */
  boolT
      WAScoplanar; /* True if qh_partitioncoplanar (qhT *qh, qh_check_maxout) */
  ...
  */
  ...
  int maxline;
  coordT* half_space;  /* malloc'd input array for halfspace
                          (qh.normal_size+coordT) */
  ...
  */
  boolT ERREXITcalled;    /* true during qh_errexit (qhT *qh, prevents duplicate
                             calls */
  boolT firstcentrum;     /* for qh_printcentrum */
  boolT old_randomdist;   /* save RANDOMdist flag during io, tracing, or
                             statistics */
  setT* coplanarfacetset; /* set of coplanar facets for searching
                             qh_findbesthorizon() */
  ...
  */
  ...
  jmp_buf rbox_errexit; /* errexit from rboxlib_r.c, defined by qh_rboxpoints()
                           only */
  char jmpXtra3[40]; /* extra bytes in case jmp_buf is defined wrong by compiler
                      */
  int rbox_isinteger;
  double rbox_out_offset;
  void* cpp_object; /* C++ pointer.  Currently used by
                       RboxPoints.qh_fprintf_rbox */
...
*/
#define FOREACHfacet_i_(qh, facets) \
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  see:
    <a href="qset_r.h#FOREACHsetelement_i_">FOREACHsetelement_i_</a>
*/
#define FOREACHneighbor_i_(qh, facet) \
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  see:
    <a href="qset_r.h#FOREACHsetelement_i_">FOREACHsetelement_i_</a>
*/
#define FOREACHpoint_i_(qh, points) \
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  see:
    <a href="qset_r.h#FOREACHsetelement_i_">FOREACHsetelement_i_</a>
*/
#define FOREACHridge_i_(qh, ridges) \
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  see:
    <a href="qset_r.h#FOREACHsetelement_i_">FOREACHsetelement_i_</a>
*/
#define FOREACHvertex_i_(qh, vertices) \
...
void qh_errprint(qhT* qh,
                 const char* string,
                 facetT* atfacet,
                 facetT* otherfacet,
                 ridgeT* atridge,
                 vertexT* atvertex);
int qh_new_qhull(qhT* qh,
                 int dim,
                 int numpoints,
                 coordT* points,
                 boolT ismalloc,
...
/***** -geom_r.c/geom2_r.c/random_r.c prototypes (duplicated from geom_r.h,
 * random_r.h) ****************/
