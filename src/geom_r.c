...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  see:
    ...
    qh_distplane [geom_r.c], QhullFacet::distance, and QhullHyperplane::distance
  are copies
*/
void qh_distplane(qhT* qh, pointT* point, facetT* facet, realT* dist) {
  ...
  if(qh->RANDOMdist) {
    randr = qh_RANDOMint;
    *dist += (2.0 * randr / qh_RANDOMmax - 1.0) * qh->RANDOMfactor *
             qh->MAXabs_coord;
  }
  if(qh->IStracing >= 4) {
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  qh_findbest(qh, point, startfacet, bestoutside, qh_ISnewfacets, qh_NOupper,
  dist, isoutside, numpart ) find facet that is furthest below a point for
  upperDelaunay facets returns facet only if !qh_NOupper and clearly above
  ...
  notes:
    If merging (testhorizon), searches horizon facets of coplanar best facets
  because after qh_distplane, this and qh_partitionpoint are the most expensive
  in 3-d avoid calls to distplane, function calls, and real number operations.
    caller traces result
    Optimized for outside points.   Tried recording a search set for
...
facetT* qh_findbest(qhT* qh,
  ...
  int numpartnew = 0;
  boolT testhorizon =
      True; /* needed if precise, e.g., rbox c D6 | qhull Q0 Tv */

  zinc_(Zfindbest);
  if(qh->IStracing >= 3 || (qh->TRACElevel && qh->TRACEpoint >= 0 &&
                            qh->TRACEpoint == qh_pointid(qh, point))) {
    if(qh->TRACElevel > qh->IStracing)
      qh->IStracing = qh->TRACElevel;
    qh_fprintf(qh, qh->ferr, 8004,
               "qh_findbest: point p%d starting at f%d isnewfacets? %d, unless "
               "%d exit if > %2.2g\n",
               qh_pointid(qh, point), startfacet->id, isnewfacets, bestoutside,
               qh->MINoutside);
    qh_fprintf(qh, qh->ferr, 8005, "  testhorizon? %d noupper? %d", testhorizon,
               noupper);
    qh_fprintf(qh, qh->ferr, 8006, "  Last point added was p%d.",
               qh->furthest_id);
  ...
  if(!startfacet->flipped) { /* test startfacet */
    *numpart = 1;
    qh_distplane(qh, point, startfacet,
                 dist); /* this code is duplicated below */
  ...
  facet = startfacet;
  while(facet) {
    trace4((qh, qh->ferr, 4001,
            "qh_findbest: neighbors of f%d, bestdist %2.2g f%d\n", facet->id,
            bestdist, getid_(bestfacet)));
    lastfacet = facet;
  ...
  if(isnewfacets) {
    if(!bestfacet) {
      bestdist = -REALmax / 2;
      bestfacet = qh_findbestnew(qh, point, startfacet->next, &bestdist,
                                 bestoutside, isoutside, &numpartnew);
      ...
      if(qh_sharpnewfacets(qh)) {
        ...
        zinc_(Zfindnewsharp);
        bestfacet = qh_findbestnew(qh, point, bestfacet, &bestdist, bestoutside,
  ...
  if(testhorizon)
    bestfacet = qh_findbesthorizon(qh, !qh_IScheckmax, point, bestfacet,
                                   noupper, &bestdist, &numpartnew);
  *dist = bestdist;
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  qh_findbesthorizon(qh, qh_IScheckmax, point, startfacet, qh_NOupper,
  &bestdist, &numpart ) search coplanar and better horizon facets from
  startfacet/bestdist ischeckmax turns off statistics and minsearch update all
  arguments must be initialized returns(ischeckmax): best facet
  returns(!ischeckmax):
  ...
  when called by qh_check_maxout() (qh_IScheckmax)
    startfacet must be closest to the point
      Otherwise, if point is beyond and below startfacet, startfacet may be a
  local minimum even though other facets are below the point. updates
  facet->maxoutside for good, visited facets may return NULL
...
facetT* qh_findbesthorizon(qhT* qh,
...
#if qh_MAXoutside
    if((!qh->ONLYgood || startfacet->good) &&
       *bestdist > startfacet->maxoutside)
      startfacet->maxoutside = *bestdist;
#endif
  }
  searchdist =
      qh_SEARCHdist; /* multiple of qh.max_outside and precision constants */
  ...
  facet = startfacet;
  while(True) {
    trace4((qh, qh->ferr, 4002,
            "qh_findbesthorizon: neighbors of f%d bestdist %2.2g f%d "
            "ischeckmax? %d noupper? %d minsearch %2.2g searchdist %2.2g\n",
      ...
      if(nextfacet) {
        if(!coplanarfacetset_size++) {
          ...
          qh_setappend(qh, &qh->coplanarfacetset,
                       nextfacet); /* Was needed for RBOX 1000 s W1e-13 P0
                      t996547055 | QHULL d Qbb Qc Tv and RBOX 1000 s Z1 G1e-13
                      t996564279 | qhull Tv  */
      }
      nextfacet = neighbor;
    } /* end of EACHneighbor */
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  notes:
    ...
    If merging (testhorizon), searches horizon facets of coplanar best facets
  because a point maybe coplanar to the bestfacet, below its horizon facet, and
  above a horizon facet of a coplanar newfacet.  For example, rbox 1000 s Z1
  G1e-13 | qhull rbox 1000 s W1e-13 P0 t992110337 | QHULL d Qbb Qc
...
facetT* qh_findbestnew(qhT* qh,
  ...
  if(!startfacet) {
    if(qh->MERGING)
      qh_fprintf(qh, qh->ferr, 6001,
                 "qhull precision error (qh_findbestnew): merging has formed "
                 "and deleted a cone of new facets.  Can not continue.\n");
    else
      qh_fprintf(qh, qh->ferr, 6002,
                 "qhull internal error (qh_findbestnew): no new facets for "
                 "point p%d\n",
                 qh->furthest_id);
  ...
  *numpart = 0;
  if(qh->IStracing >= 3 || (qh->TRACElevel && qh->TRACEpoint >= 0 &&
    ...
    qh_fprintf(qh, qh->ferr, 8009, "  Last point added p%d visitid %d.",
  ...
  if(testhorizon || !bestfacet) /* testhorizon is always True.  Keep the same
                                   code as qh_findbest */
    bestfacet = qh_findbesthorizon(qh, !qh_IScheckmax, point,
                                   bestfacet ? bestfacet : startfacet,
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  notes:
     ...
     see Golub & van Loan, 1983, Eq. 4.4-9 for "Gaussian elimination with
  complete pivoting"
...
void qh_setfacetplane(qhT* qh, facetT* facet) {
  ...
  if(facet == qh->tracefacet) {
    ...
    qh_fprintf(qh, qh->ferr, 8012, "qh_setfacetplane: facet f%d created.\n",
               facet->id);
    qh_fprintf(qh, qh->ferr, 8013, "  Last point added to hull was p%d.",
               qh->furthest_id);
    if(zzval_(Ztotmerge))
      qh_fprintf(qh, qh->ferr, 8014, "  Last merge was #%d.",
                 zzval_(Ztotmerge));
  ...
  if(qh->hull_dim <= 4) {
    ...
    if(qh->RANDOMdist) {
      ...
      FOREACHvertex_(facet->vertices) {
        ...
        for(k = qh->hull_dim; k--;)
          *(gmcoord++) =
              *coord++ * qh_randomfactor(qh, qh->RANDOMa, qh->RANDOMb);
      }
    } else {
  ...
  if(qh->hull_dim > 4 || nearzero) {
    ...
    qh_sethyperplane_gauss(qh, qh->hull_dim, qh->gm_row, point0,
    ...
    FOREACHvertex_(facet->vertices) {
      if(vertex->point != point0) {
        ...
        if(dist > wwval_(Wnewvertexmax)) {
          ...
          istrace = True;
        if(istrace) {
          qh_fprintf(qh, qh->ferr, 8016,
                     "qh_setfacetplane: ====== vertex p%d(v%d) increases "
                     "max_outside to %2.2g for new facet f%d last p%d\n",
  ...
  if(qh->IStracing >= 3) {
    qh_fprintf(qh, qh->ferr, 8017,
               "qh_setfacetplane: f%d offset %2.2g normal: ", facet->id,
               facet->offset);
    for(k = 0; k < qh->hull_dim; k++)
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  qh_sethyperplane_det(qh, dim, rows, point0, toporient, normal, offset,
  nearzero ) given dim X dim array indexed by rows[], one row per point,
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  qh_sethyperplane_gauss(qh, dim, rows, point0, toporient, normal, offset,
  nearzero ) given(dim-1) X dim array of rows[i]= V_{i+1} - V_0 (point0) set
