  ...
  FORALLnew_facets {
    ...
    if(horizon->simplicial) {
      ...
      if(visible) {
        ...
        qh_fprintf(qh, qh->ferr, 6102,
                   "qhull internal error (qh_attachnewfacets): couldn't find "
                   "visible facet for horizon f%d of newfacet f%d\n",
...
void qh_deletevisible(qhT* qh /*qh.visible_list*/) {
  ...
  if(numvisible != qh->num_visible) {
    qh_fprintf(qh, qh->ferr, 6103,
               "qhull internal error (qh_deletevisible): qh->num_visible %d is "
               "not number of visible facets %d\n",
...
setT* qh_facetintersect(qhT* qh,
  ...
  if(i >= dim || j >= dim) {
    qh_fprintf(qh, qh->ferr, 6104,
               "qhull internal error (qh_facetintersect): f%d or f%d not in "
               "others neighbors\n",
               facetA->id, facetB->id);
    qh_errexit2(qh, qh_ERRqhull, facetA, facetB);
  }
  intersect = qh_setnew_delnthsorted(qh, facetA->vertices, qh->hull_dim, *skipA,
                                     prepend);
  trace4((qh, qh->ferr, 4047,
...
#pragma warning(disable : 4311) /* 'type cast': pointer truncation from \
  ...
  if(hashsize < 0) {
    qh_fprintf(qh, qh->ferr, 6202,
               "qhull internal error: negative hashsize %d passed to "
               "qh_gethash [poly.c]\n",
               hashsize);
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  non-simplicial visible facet) (deletes ridge if neighbor is simplicial)
  ...
  FOREACHridge_(visible->ridges) {
    ...
    if(neighbor->visible) {
      ...
      if(qh->ONLYgood) {
        ...
        if(neighbor->seen) {
          if(neighbor->simplicial) {
            qh_fprintf(qh, qh->ferr, 6105,
                       "qhull internal error (qh_makenew_nonsimplicial): "
                       "simplicial f%d sharing two ridges with f%d\n",
...
facetT* qh_makenew_simplicial(qhT* qh,
  ...
  FOREACHneighbor_(visible) {
    if(!neighbor->seen && !neighbor->visible) {
      vertices = qh_facetintersect(qh, neighbor, visible, &horizonskip,
                                   &visibleskip, 1);
  ...
  for(scan = hash; (facet = SETelemt_(qh->hash_table, scan, facetT));
    ...
    if(qh_matchvertices(qh, 1, newfacet->vertices, newskip, facet->vertices,
      ...
      if(SETelem_(newfacet->vertices, newskip) ==
        ...
        qh_fprintf(qh, qh->ferr, 6106,
                   "qhull precision error: Vertex sets are the same for f%d "
                   "and f%d.  Can not force output.\n",
      ...
      if(!qh->PREmerge && !qh->MERGEexact) {
        ...
        qh_fprintf(qh, qh->ferr, 6107,
                   "qhull precision error: facets f%d, f%d and f%d meet at a "
                   "ridge with more than 2 neighbors.  Can not continue.\n",
      ...
      if(matchfacet != qh_DUPLICATEridge) {
        ...
        if(matchfacet) {
          ...
          if(matchskip < 0) {
            qh_fprintf(
                qh, qh->ferr, 6260,
                "qhull internal error (qh_matchneighbor): matchfacet f%d is in "
                "f%d neighbors but not vice versa.  Can not continue.\n",
...
void qh_matchnewfacets(qhT* qh /* qh.newfacet_list */) {
  ...
  qh_newhashtable(
      qh, numnew * (qh->hull_dim - 1)); /* twice what is normally needed,
  ...
  if(hashcount) {
    qh_fprintf(qh, qh->ferr, 6108,
               "qhull internal error (qh_matchnewfacets): %d neighbors did not "
               "match up\n",
               hashcount);
  ...
  if(qh->IStracing >= 2) {
    ...
    qh_fprintf(qh, qh->ferr, 8089,
               "qh_matchnewfacets: %d new facets, %d unused hash entries .  "
               "hashsize %d\n",
...
/*-<a                             href="qh-poly_r.htm#TOC"
  ...
  qh_pointid(qh, point )
    ...
    returns qh_IDnone(-3) if null, qh_IDinterior(-2) if interior, or
  qh_IDunknown(-1) if not known
...
*/
void qh_updatevertices(
    qhT* qh /*qh.newvertex_list, newfacet_list, visible_list*/) {
  facetT *newfacet = NULL, *neighbor, **neighborp, *visible;
  vertexT *vertex, **vertexp;

  trace3((qh, qh->ferr, 3013,
