/*<html><pre>  -<a                             href="qh-mem_r.htm"
  ...
  To initialize memory:
    ...
    qh_meminitbuffers(qh, qh->IStracing, qh_MEMalign, 7,
  qh_MEMbufsize,qh_MEMinitbuf); qh_memsize(qh, (int)sizeof(facetT));
...
void* qh_memalloc(qhT* qh, int insize) {
  ...
  if(insize < 0) {
    qh_fprintf(qh, qh->qhmem.ferr, 6235,
               "qhull error (qh_memalloc): negative request size (%d).  Did "
               "int overflow due to high-D?\n",
               insize); /* WARN64 */
  ...
  if(insize >= 0 && insize <= qh->qhmem.LASTsize) {
    ...
    if((object = *freelistp)) {
      ...
      *freelistp =
      ...
      if(qh->qhmem.IStracing >= 5)
        qh_fprintf(
            qh, qh->qhmem.ferr, 8141,
            "qh_mem %p n %8d alloc quick: %d bytes (tot %d cnt %d)\n", object,
            n, outsize, qh->qhmem.totshort,
      ...
      if(outsize > qh->qhmem.freesize) {
        ...
        if(!(newbuffer = qh_malloc((size_t)bufsize))) {
          qh_fprintf(qh, qh->qhmem.ferr, 6080,
                     "qhull error (qh_memalloc): insufficient memory to "
                     "allocate short memory buffer (%d bytes)\n",
                     bufsize);
          qh_errexit(qh, qhmem_ERRmem, NULL, NULL);
        }
        *((void**)newbuffer) =
            qh->qhmem.curbuffer; /* prepend newbuffer to curbuffer
        ...
        /* Periodically test totbuffer.  It matches at beginning and exit of
         * every call */
        n = qh->qhmem.totshort + qh->qhmem.totfree + qh->qhmem.totdropped +
            qh->qhmem.freesize - outsize;
        if(qh->qhmem.totbuffer != n) {
          qh_fprintf(qh, qh->qhmem.ferr, 6212,
                     "qh_memalloc internal error: short totbuffer %d != "
                     "totshort+totfree... %d\n",
    ...
    if(!(object = qh_malloc((size_t)outsize))) {
      qh_fprintf(qh, qh->qhmem.ferr, 6082,
                 "qhull error (qh_memalloc): insufficient memory to allocate "
                 "%d bytes\n",
                 outsize);
      qh_errexit(qh, qhmem_ERRmem, NULL, NULL);
    }
    if(qh->qhmem.IStracing >= 5)
      qh_fprintf(qh, qh->qhmem.ferr, 8057,
                 ...
                 object, qh->qhmem.cntlong + qh->qhmem.freelong, outsize,
                 qh->qhmem.totlong, qh->qhmem.cntlong - qh->qhmem.freelong);
  }
  return (object);
} /* memalloc */
...
void qh_memcheck(qhT* qh) {
  ...
  if(!qh) {
    qh_fprintf_stderr(
        6243, "qh_memcheck(qh) error: qh is 0.  It does not point to a qhT");
    qh_exit(qhmem_ERRqhull); /* can not use qh_errexit() */
  }
  if(qh->qhmem.ferr == 0 || qh->qhmem.IStracing < 0 ||
     qh->qhmem.IStracing > 10 ||
     (((qh->qhmem.ALIGNmask + 1) & qh->qhmem.ALIGNmask) != 0)) {
    qh_fprintf_stderr(
        6244,
        "qh_memcheck error: either qh->qhmem is overwritten or qh->qhmem is "
        "not initialized.  Call qh_mem_new() or qh_new_qhull() before calling "
        "qh_mem routines.  ferr 0x%x IsTracing %d ALIGNmask 0x%x",
  ...
  if(qh->qhmem.IStracing != 0)
    qh_fprintf(
        ...
        "segmentation fault indicates an overwrite of qh->qhmem\n");
  ...
  if(totfree != qh->qhmem.totfree) {
    qh_fprintf(qh, qh->qhmem.ferr, 6211,
               "Qhull internal error (qh_memcheck): totfree %d not equal to "
               "freelist total %d\n",
               qh->qhmem.totfree, totfree);
    qh_errexit(qh, qhmem_ERRqhull, NULL, NULL);
  }
  if(qh->qhmem.IStracing != 0)
    qh_fprintf(qh, qh->qhmem.ferr, 8144,
               "qh_memcheck: total size of freelists totfree is the same as "
               "qh->qhmem.totfree\n",
               totfree);
} /* memcheck */
    ...
    *freelistp = object;
    ...
    if(qh->qhmem.IStracing >= 5)
      qh_fprintf(qh, qh->qhmem.ferr, 8142,
                 ...
                 object, idx, outsize, qh->qhmem.totshort,
                 qh->qhmem.cntshort + qh->qhmem.cntquick - qh->qhmem.freeshort);
#endif
  } else {
    qh->qhmem.freelong++;
    ...
    if(qh->qhmem.IStracing >= 5)
      qh_fprintf(qh, qh->qhmem.ferr, 8058,
                 ...
                 object, qh->qhmem.cntlong + qh->qhmem.freelong, insize,
                 qh->qhmem.totlong, qh->qhmem.cntlong - qh->qhmem.freelong);
    qh_free(object);
  }
} /* memfree */
...
void qh_memfreeshort(qhT* qh, int* curlong, int* totlong) {
  ...
  *totlong = qh->qhmem.totlong;
  ...
  ferr = qh->qhmem.ferr;
  memset((char*)&qh->qhmem, 0,
         sizeof(qh->qhmem)); /* every field is 0, FALSE, NULL */
  qh->qhmem.ferr = ferr;
} /* memfreeshort */
...
void qh_meminit(qhT* qh, FILE* ferr) {
  ...
  if(sizeof(void*) < sizeof(int)) {
    qh_fprintf(qh, qh->qhmem.ferr, 6083,
               "qhull internal error (qh_meminit): sizeof(void*) %d < "
               "sizeof(int) %d.  qset.c will not work\n",
               (int)sizeof(void*), (int)sizeof(int));
    qh_exit(qhmem_ERRqhull); /* can not use qh_errexit() */
  }
  if(sizeof(void*) > sizeof(ptr_intT)) {
    qh_fprintf(qh, qh->qhmem.ferr, 6084,
               "qhull internal error (qh_meminit): sizeof(void*) %d > "
               "sizeof(ptr_intT) %d. Change ptr_intT in mem.h to 'long long'\n",
...
void qh_meminitbuffers(qhT* qh,
  ...
  if(qh->qhmem.ALIGNmask & ~qh->qhmem.ALIGNmask) {
    qh_fprintf(qh, qh->qhmem.ferr, 6085,
               "qhull internal error (qh_meminit): memory alignment %d is not "
               "a power of 2\n",
               alignment);
    qh_errexit(qh, qhmem_ERRqhull, NULL, NULL);
  }
  ...
  if(!qh->qhmem.sizetable || !qh->qhmem.freelists) {
    qh_fprintf(qh, qh->qhmem.ferr, 6086,
...
void qh_memsetup(qhT* qh) {
  ...
  qsort(qh->qhmem.sizetable, (size_t)qh->qhmem.TABLEsize, sizeof(int),
        qh_intcompare);
  qh->qhmem.LASTsize = qh->qhmem.sizetable[qh->qhmem.TABLEsize - 1];
  if(qh->qhmem.LASTsize >= qh->qhmem.BUFsize ||
     qh->qhmem.LASTsize >= qh->qhmem.BUFinit) {
    qh_fprintf(qh, qh->qhmem.ferr, 6087,
               "qhull error (qh_memsetup): largest mem size %d is >= buffer "
               "size %d or initial buffer size %d\n",
               qh->qhmem.LASTsize, qh->qhmem.BUFsize, qh->qhmem.BUFinit);
    qh_errexit(qh, qhmem_ERRmem, NULL, NULL);
  }
  if(!(qh->qhmem.indextable =
           (int*)qh_malloc((qh->qhmem.LASTsize + 1) * sizeof(int)))) {
    qh_fprintf(qh, qh->qhmem.ferr, 6088,
...
%7d bytes per short memory buffer (initially %d bytes)\n",
             ...
             qh->qhmem.freeshort, qh->qhmem.freelong, qh->qhmem.totshort,
             qh->qhmem.totfree, qh->qhmem.totdropped + qh->qhmem.freesize,
             qh->qhmem.totunused, qh->qhmem.maxlong, qh->qhmem.totlong,
             qh->qhmem.cntlong - qh->qhmem.freelong, qh->qhmem.totbuffer,
             qh->qhmem.BUFsize, qh->qhmem.BUFinit);
  if(qh->qhmem.cntlarger) {
    qh_fprintf(qh, fp, 9279,
               "%7d calls to qh_setlarger\n%7.2g     average copy size\n",
               qh->qhmem.cntlarger,
               ((float)qh->qhmem.totlarger) / (float)qh->qhmem.cntlarger);
...
void* qh_memalloc(qhT* qh, int insize) {
  ...
  if(!(object = qh_malloc((size_t)insize))) {
    qh_fprintf(qh, qh->qhmem.ferr, 6090,
  ...
  if(qh->qhmem.IStracing >= 5)
    qh_fprintf(qh, qh->qhmem.ferr, 8060,
               "qh_mem %p n %8d alloc long: %d bytes (tot %d cnt %d)\n", object,
               qh->qhmem.cntlong + qh->qhmem.freelong, insize,
               qh->qhmem.totlong, qh->qhmem.cntlong - qh->qhmem.freelong);
  return object;
}
...
void qh_meminit(qhT* qh, FILE* ferr) {
  ...
  if(ferr)
    ...
    qh->qhmem.ferr = stderr;
  if(sizeof(void*) < sizeof(int)) {
    qh_fprintf(qh, qh->qhmem.ferr, 6091,
               "qhull internal error (qh_meminit): sizeof(void*) %d < "
               "sizeof(int) %d.  qset.c will not work\n",
               (int)sizeof(void*), (int)sizeof(int));
    qh_errexit(qh, qhmem_ERRqhull, NULL, NULL);
...
%7d bytes of long memory allocated (max, except for input)\n\
%7d bytes of long memory in use (in %d pieces)\n",
             qh->qhmem.cntlong, qh->qhmem.freelong, qh->qhmem.maxlong,
             qh->qhmem.totlong, qh->qhmem.cntlong - qh->qhmem.freelong);
}
...
>-------------------------------</a><a name="memtotlong">-</a>
  ...
  returns:
    ...
    Returns the maximum long memory and total short buffer (minus one link per
buffer) Does not error (for deprecated UsingLibQhull.cpp (libqhullpcpp))
*/
