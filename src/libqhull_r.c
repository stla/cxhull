...
boolT qh_addpoint(qhT* qh, pointT* furthest, facetT* facet, boolT checkdist) {
  ...
  if(!facet) {
    qh_fprintf(qh, qh->ferr, 6213,
               "qhull internal error (qh_addpoint): NULL facet.  Need to call "
               "qh_findbestfacet first\n");
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  }
  if(checkdist) {
    facet = qh_findbest(qh, furthest, facet, !qh_ALL, !qh_ISnewfacets,
                        !qh_NOupper, &dist, &isoutside, &numpart);
  ...
  vertex =
      qh_makenewfacets(qh, furthest /*visible_list, attaches if !ONLYgood */);
  ...
  if(qh->ONLYgood && !qh_findgood(qh, qh->newfacet_list, goodhorizon) &&
     !qh->GOODclosest) {
    FORALLnew_facets qh_delfacet(qh, newfacet);
...
void qh_build_withrestart(qhT* qh) {
  ...
  while(True) {
    ...
    if(restart) {                      /* only from qh_precision() */
      ...
      qh->STOPcone =
          qh_IDunknown; /* if break from joggle, prevents normal output */
    }
    if(!qh->RERUN && qh->JOGGLEmax < REALmax / 2) {
    ...
    if(qh->build_cnt == qh->RERUN) {
      qh->IStracing =
          qh->TRACElastrun; /* duplicated from qh_initqhull_globals */
      if(qh->TRACEpoint != qh_IDunknown || qh->TRACEdist < REALmax / 2 ||
         qh->TRACEmerge) {
...
void qh_buildhull(qhT* qh) {
  ...
  FORALLfacets {
    if(facet->visible || facet->newfacet) {
      qh_fprintf(qh, qh->ferr, 6165,
                 "qhull internal error (qh_buildhull): visible or new facet "
                 "f%d in facet list\n",
                 facet->id);
  ...
  FORALLvertices {
    if(vertex->newlist) {
      qh_fprintf(qh, qh->ferr, 6166,
                 "qhull internal error (qh_buildhull): new vertex f%d in "
                 "vertex list\n",
                 vertex->id);
  ...
  if(qh->num_outside && !furthest) {
    qh_fprintf(qh, qh->ferr, 6167,
               "qhull internal error (qh_buildhull): %d outside points were "
               "never processed.\n",
               qh->num_outside);
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  }
  trace1(
      (qh, qh->ferr, 1039, "qh_buildhull: completed the hull construction\n"));
} /* buildhull */
...
At %02d:%02d:%02d & %2.5g CPU secs, qhull has created %d facets and merged %d.\n\
 ...
 outside points.  Next is point p%d(v%d), %2.2g above f%d.\n",
    ...
    qh_fprintf(qh, qh->ferr, 8120,
               "qh_addpoint: add p%d(v%d) to hull of %d facets(%2.2g above "
               "f%d) and %d outside at %4.4g CPU secs.  Previous was p%d.\n",
  ...
  *goodvisible = *goodhorizon = 0;
  ...
  FORALLvisible_facets {
    if(visible->tricoplanar && !qh->TRInormals) {
      qh_fprintf(qh, qh->ferr, 6230,
                 "Qhull internal error (qh_findhorizon): does not work for "
...
QhullPoint p%d was above all facets.\n",
  ...
  trace1((qh, qh->ferr, 1041,
          "qh_findhorizon: %d horizon facets(good %d), %d visible(good %d), %d "
          "coplanar\n",
...
pointT* qh_nextfurthest(qhT* qh, facetT** visible) {
  ...
  while((facet = qh->facet_next) != qh->facet_tail) {
    ...
    if(qh->RANDOMoutside) {
      ...
      qh_fprintf(qh, qh->ferr, 6169,
                 "qhull internal error (qh_nextfurthest): num_outside %d is "
...
void qh_partitionall(qhT* qh, setT* vertices, pointT* points, int numpoints) {
  ...
  if(!qh->BESToutside) { /* matches conditional for qh_partitionpoint below */
    distoutside = qh_DISToutside; /* multiple of qh.MINoutside & qh.max_outside,
                                     see user.h */
    zval_(Ztotpartition) =
        qh->num_points - qh->hull_dim - 1; /*misses GOOD... */
...
/*-<a                             href="qh-qhull_r.htm#TOC"
  ...
  set else if point is clearly outside of qh.max_outside and
  bestfacet->coplanarset and bestfacet is more than perpendicular to facet
      repartition the point using qh_findbest() -- it may be put on an
  outsideset else update qh.max_outside
*/
void qh_partitioncoplanar(qhT* qh, pointT* point, facetT* facet, realT* dist) {
  ...
  if(!dist) {
    if(qh->findbestnew)
      bestfacet = qh_findbestnew(qh, point, facet, &bestdist, qh_ALL,
                                 &isoutside, &numpart);
    else
      bestfacet = qh_findbest(qh, point, facet, qh_ALL, !qh_ISnewfacets,
                              qh->DELAUNAY, &bestdist, &isoutside, &numpart);
    zinc_(Ztotpartcoplanar);
    zzadd_(Zpartcoplanar, numpart);
    if(!qh->DELAUNAY &&
       !qh->KEEPinside) { /*  for 'd', bestdist skips upperDelaunay facets */
      if(qh->KEEPnearinside) {
        if(bestdist < -qh->NEARinside) {
          zinc_(Zcoplanarinside);
          trace4((qh, qh->ferr, 4062,
                  "qh_partitioncoplanar: point p%d is more than near-inside "
                  "facet f%d dist %2.2g findbestnew %d\n",
                  qh_pointid(qh, point), bestfacet->id, bestdist,
                  qh->findbestnew));
          return;
        }
      } else if(bestdist < -qh->MAXcoplanar) {
        trace4((qh, qh->ferr, 4063,
                "qh_partitioncoplanar: point p%d is inside facet f%d dist "
                "%2.2g findbestnew %d\n",
                qh_pointid(qh, point), bestfacet->id, bestdist,
                qh->findbestnew));
        zinc_(Zcoplanarinside);
  ...
  if(bestdist > qh->max_outside) {
    if(!dist && facet != bestfacet) {
      ...
      if(angle < 0) {
        ...
        zinc_(Zpartflip);
        trace2((qh, qh->ferr, 2058,
                "qh_partitioncoplanar: repartition point p%d from f%d.  It is "
                "above flipped facet f%d dist %2.2g\n",
    ...
    if(bestdist > qh->TRACEdist) {
      qh_fprintf(qh, qh->ferr, 8122,
                 "qh_partitioncoplanar: ====== p%d from f%d increases "
                 "max_outside to %2.2g of f%d last p%d\n",
                 qh_pointid(qh, point), facet->id, bestdist, bestfacet->id,
                 qh->furthest_id);
...
void qh_partitionpoint(qhT* qh, pointT* point, facetT* facet) {
  ...
  if(qh->findbestnew)
    bestfacet = qh_findbestnew(qh, point, facet, &bestdist, qh->BESToutside,
                               &isoutside, &numpart);
  else
    bestfacet = qh_findbest(qh, point, facet, qh->BESToutside, qh_ISnewfacets,
                            !qh_NOupper, &bestdist, &isoutside, &numpart);
  zinc_(Ztotpartition);
  zzadd_(Zpartition, numpart);
  if(qh->NARROWhull) {
  ...
  if(isoutside) {
    ...
    qh->num_outside++;
    trace4((qh, qh->ferr, 4065,
            "qh_partitionpoint: point p%d is outside facet f%d new? %d (or "
            "narrowhull)\n",
            qh_pointid(qh, point), bestfacet->id, bestfacet->newfacet));
  } else if(qh->DELAUNAY ||
            bestdist >= -qh->MAXcoplanar) { /* for 'd', bestdist skips
...
*/
void qh_partitionvisible(qhT* qh /*qh.visible_list*/,
                         boolT allpoints,
                         int* numoutside) {
  ...
  *numoutside = 0;
  FORALLvisible_facets {
    ...
    if(newfacet == qh->facet_tail) {
      qh_fprintf(qh, qh->ferr, 6170,
                 "qhull precision error (qh_partitionvisible): all new facets "
...
void qh_printsummary(qhT* qh, FILE* fp) {
  ...
  if(qh->STOPcone || qh->STOPpoint)
    qh_fprintf(qh, fp, 9288,
               "\nAt a premature exit due to 'TVn', 'TCn', 'TRn', or precision "
               "error with 'QJn'.");
  if(qh->UPPERdelaunay)
...
Convex hull of %d points in %d-d:\n\n",
  ...
  if(numtricoplanars)
    qh_fprintf(qh, fp, 9318, "  Number of triangulated facets: %d\n",
               numtricoplanars);
  qh_fprintf(qh, fp, 9319, "\nStatistics for: %s | %s", qh->rbox_command,
             qh->qhull_command);
  ...
  qh_fprintf(qh, fp, 9325, "  Number of distance tests for qhull: %d\n",
             zzval_(Zpartition) + zzval_(Zpartitionall) +
                 zzval_(Znumvisibility) + zzval_(Zpartcoplanar));
  ...
  if(qh->MERGING) {
    ...
    if(outerplane > 2 * qh->DISTround) {
      ...
      if(ratio > 0.05 && 2 * qh->ONEmerge > qh->MINoutside &&
