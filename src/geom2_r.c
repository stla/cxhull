...
coordT* qh_copypoints(qhT* qh, coordT* points, int numpoints, int dimension) {
  ...
  if(!(newpoints = (coordT*)qh_malloc((size_t)size))) {
    qh_fprintf(qh, qh->ferr, 6004,
               "qhull error: insufficient memory to copy %d points\n",
               numpoints);
    qh_errexit(qh, qh_ERRmem, NULL, NULL);
  }
  memcpy((char*)newpoints, (char*)points,
         (size_t)size); /* newpoints!=0 by QH6004 */
  return newpoints;
} /* copypoints */
  ...
  *nearzero = False;
  if(dim < 2) {
    qh_fprintf(qh, qh->ferr, 6005,
               "qhull internal error (qh_determinate): only implemented for "
               "dimension >= 2\n");
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  } else if(dim == 2) {
    det = det2_(rows[0][0], rows[0][1], rows[1][0], rows[1][1]);
    if(fabs_(det) <
       10 * qh->NEARzero[1]) /* not really correct, what should this be? */
      *nearzero = True;
  } else if(dim == 3) {
    det = det3_(rows[0][0], rows[0][1], rows[0][2], rows[1][0], rows[1][1],
                rows[1][2], rows[2][0], rows[2][1], rows[2][2]);
    if(fabs_(det) < 10 * qh->NEARzero[2]) /* what should this be?  det 5.5e-12
                                             was flat for qh_maxsimplex of
                                             qdelaunay 0,0 27,27 -36,36 -9,63 */
      *nearzero = True;
  } else {
    qh_gausselim(qh, rows, dim, dim, &sign,
...
void qh_detroundoff(qhT* qh) {
  ...
  if(!qh->SETroundoff) {
    qh->DISTround =
        qh_distround(qh, qh->hull_dim, qh->MAXabs_coord, qh->MAXsumcoord);
    if(qh->RANDOMdist)
  ...
  if(qh->RANDOMdist && qh->POSTmerge)
    qh_option(qh, "Centrum-postmerge-with-random", NULL,
...
realT qh_detsimplex(qhT* qh,
  ...
  trace2((qh, qh->ferr, 2002,
          "qh_detsimplex: det=%2.2g for point p%d, dim %d, nearzero? %d\n", det,
          qh_pointid(qh, apex), dim, *nearzero));
  return det;
} /* detsimplex */
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  notes:
    calculate roundoff error according to Golub & van Loan, 1983, Lemma 3.2-1,
  "Rounding Errors" use sqrt(dim) since one vector is normalized or use
  maxsumabs since one vector is < 1
*/
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  notes:
    ...
    projecting tricoplanar facets to the hyperplane does not appear to make a
  difference
...
realT qh_facetarea(qhT* qh, facetT* facet) {
  ...
  if(facet->simplicial) {
    ...
    area = qh_facetarea_simplex(qh, qh->hull_dim, apex->point, facet->vertices,
                                apex, facet->toporient, facet->normal,
                                &facet->offset);
  } else {
    ...
    FOREACHridge_(facet->ridges) area += qh_facetarea_simplex(
        qh, qh->hull_dim, centrum, ridge->vertices, NULL,
  ...
  trace4(
      (qh, qh->ferr, 4009, "qh_facetarea: f%d area %2.2g\n", facet->id, area));
  return area;
} /* facetarea */
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  qh_facetarea_simplex(qh, dim, apex, vertices, notvertex, toporient, normal,
  offset ) return area for a simplex defined by an apex, a base of vertices, an
  orientation, and a unit normal if simplicial or tricoplanar facet, notvertex
  is defined and it is skipped in vertices
...
realT qh_facetarea_simplex(qhT* qh,
  ...
  if(i != dim - 1) {
    qh_fprintf(qh, qh->ferr, 6008,
               "qhull internal error (qh_facetarea_simplex): #points %d != dim "
               "%d -1\n",
               i, dim);
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  }
...
void qh_joggleinput(qhT* qh) {
  ...
  if(!qh->input_points) { /* first call */
    ...
    if(qh->JOGGLEmax == 0.0) {
      qh->JOGGLEmax =
          qh_detjoggle(qh, qh->input_points, qh->num_points, qh->hull_dim);
  ...
  if(qh->build_cnt > 1 && qh->JOGGLEmax > fmax_(qh->MAXwidth / 4, 0.1)) {
    qh_fprintf(qh, qh->ferr, 6010,
               "qhull error: the current joggle for 'QJn', %.2g, is too large "
               "for the width\nof the input.  If possible, recompile Qhull "
               "with higher-precision reals.\n",
               qh->JOGGLEmax);
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  }
  ...
  if(sizinit < 2) {
    ...
    if(sizinit < 2) {
      ...
      if(zzval_(Zsetplane) > qh->hull_dim + 1) {
        qh_fprintf(qh, qh->ferr, 6012,
                   "qhull precision error (qh_maxsimplex for "
                   "voronoi_center):\n%d points with the same x coordinate.\n",
                   qh_setsize(qh, maxpoints) + numpoints);
        qh_errexit(qh, qh_ERRprec, NULL, NULL);
      } else {
        qh_fprintf(qh, qh->ferr, 6013,
                   "qhull input error: input is less than %d-dimensional since "
...
void qh_projectinput(qhT* qh) {
  ...
  if(!(newpoints = qh->temp_malloc =
           (coordT*)qh_malloc(newnum * newdim * sizeof(coordT)))) {
    qh_memfree(qh, project, projectsize);
    qh_fprintf(qh, qh->ferr, 6016,
  ...
  trace1((qh, qh->ferr, 1003,
          "qh_projectinput: updating lower and upper_bound\n"));
  qh_projectpoints(qh, project, qh->input_dim + 1, qh->lower_bound, 1,
                   qh->input_dim + 1, qh->lower_bound, newdim + 1);
  qh_projectpoints(qh, project, qh->input_dim + 1, qh->upper_bound, 1,
                   qh->input_dim + 1, qh->upper_bound, newdim + 1);
  if(qh->HALFspace) {
    if(!qh->feasible_point) {
      qh_memfree(qh, project, projectsize);
      qh_fprintf(qh, qh->ferr, 6017,
                 "qhull internal error (qh_projectinput): HALFspace defined "
                 "without qh.feasible_point\n");
      qh_errexit(qh, qh_ERRqhull, NULL, NULL);
    }
    qh_projectpoints(qh, project, qh->input_dim, qh->feasible_point, 1,
                     qh->input_dim, qh->feasible_point, newdim);
  }
  qh_memfree(qh, project, projectsize);
  if(qh->POINTSmalloc)
...
void qh_projectpoints(qhT* qh,
  ...
  for(k = 0; k < n; k++)
    testdim += project[k];
  if(testdim != newdim) {
    qh_fprintf(qh, qh->ferr, 6018,
               "qhull internal error (qh_projectpoints): newdim %d should be "
               "%d after projection\n",
               newdim, testdim);
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  }
...
void qh_rotateinput(qhT* qh, realT** rows) {
  if(!qh->POINTSmalloc) {
    qh->first_point =
        qh_copypoints(qh, qh->first_point, qh->num_points, qh->hull_dim);
    qh->POINTSmalloc = True;
  }
...
void qh_rotatepoints(qhT* qh,
  ...
  if(qh->IStracing >= 1)
    qh_printmatrix(qh, qh->ferr, "qh_rotatepoints: rotate points by", row, dim,
                   dim);
...
void qh_scalelast(qhT* qh,
  ...
  if(nearzero) {
    if(qh->DELAUNAY)
      qh_fprintf(qh, qh->ferr, 6019,
                 "qhull input error: can not scale last coordinate.  Input is "
                 "cocircular\n   or cospherical.   Use option 'Qz' to add a "
                 "point at infinity.\n");
    else
      qh_fprintf(qh, qh->ferr, 6020,
                 "qhull input error: can not scale last coordinate.  New "
                 "bounds [0, %2.2g] are too wide for\nexisting bounds [%2.2g, "
                 "%2.2g] (width %2.2g)\n",
...
void qh_scalepoints(qhT* qh,
  ...
  for(k = 0; k < dim; k++) {
    ...
    if(qh->DELAUNAY && k == dim - 1 && newhigh < newlow) {
      qh_fprintf(qh, qh->ferr, 6021,
                 "qhull input error: 'Qb%d' or 'QB%d' inverts paraboloid since "
                 "high bound %.2g < low bound %.2g\n",
                 k, k, newhigh, newlow);
      qh_errexit(qh, qh_ERRinput, NULL, NULL);
    }
    scale = qh_divzero(newhigh - newlow, high - low, qh->MINdenom_1, &nearzero);
    if(nearzero) {
      qh_fprintf(qh, qh->ferr, 6022,
                 "qhull input error: %d'th dimension's new bounds [%2.2g, "
...
void qh_setdelaunay(qhT* qh, int dim, int count, pointT* points) {
  ...
  if(qh->last_low < REALmax / 2)
    qh_scalelast(qh, points, count, dim, qh->last_low, qh->last_high,
                 qh->last_newhigh);
} /* setdelaunay */
...
/*-<a                             href="qh-geom_r.htm#TOC"
  ...
  qh_sethalfspace_all(qh, dim, count, halfspaces, feasible )
    ...
    feasible is a point inside all halfspaces
...
coordT* qh_sethalfspace_all(qhT* qh,
  ...
  for(i = 0; i < count; i++) {
    ...
    if(!qh_sethalfspace(qh, newdim, coordp, &coordp, normalp, offsetp,
                        feasible)) {
      qh_free(newpoints); /* feasible is not inside halfspace as reported by
                             qh_sethalfspace */
...
pointT* qh_voronoi_center(qhT* qh, int dim, setT* points) {
  ...
  if(size == dim + 1)
    ...
    qh_fprintf(qh, qh->ferr, 6025,
               "qhull internal error (qh_voronoi_center):\n  need at least %d "
               "points to construct a Voronoi center\n",
               dim + 1);
    qh_errexit(qh, qh_ERRqhull, NULL, NULL);
  ...
  if(infinite) {
    ...
    if(qh->IStracing)
      qh_printpoints(qh, qh->ferr, "qh_voronoi_center: at infinity for ",
                     simplex);
  } else {
    for(i = 0; i < dim; i++) {
      gmcoord = qh->gm_matrix;
      ...
      center[i] =
          qh_determinant(qh, qh->gm_row, dim, &nearzero) * factor + point0[i];
    }
#ifndef qh_NOtrace
    if(qh->IStracing >= 3) {
      qh_fprintf(qh, qh->ferr, 8033,
      ...
      if(qh->IStracing >= 5) {
        ...
        FOREACHpoint_(simplex)
            qh_fprintf(qh, qh->ferr, 8034, "p%d dist %.2g, ",
                       qh_pointid(qh, point), qh_pointdist(point, center, dim));
